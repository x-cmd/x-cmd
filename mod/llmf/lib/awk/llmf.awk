...
function llmf_gen_unit_str( role, content ){
    if ( role !~ "^\"" )    role = jqu( role )
    if ( content !~ "^\"" ) content = jqu( content )
    return "{ \"role\": " role ", \"content\": " content " }"
}

function llmf_gen_history_str( history_obj, i,        _text_req, _text_res, _text_finishReason ){
    ...
    return  llmf_gen_unit_str( "user", _text_req )", " llmf_gen_unit_str( "assistant", _text_res )
}

function llmf_gen_minion_content_str(minion_obj, minion_kp,      context, example, content){
    ...
    return llmf_gen_unit_str( "user", context example content )
}

function llmf_req_from_creq(history_obj, minion_obj, minion_kp,          i, l, str, \
    ...
    for (i=1; i<=l; ++i){
        str = llmf_gen_history_str(history_obj, i)
        if(str != "") _history_str = _history_str str " ,"
    }

    _system_str = minion_system_tostr(minion_obj, minion_kp)
    if (_system_str != "") _system_str = llmf_gen_unit_str( "system", _system_str ) " ,"

    _filelist_str   = minion_filelist_attach( minion_obj, minion_kp )
    if (_filelist_str != "") _filelist_str = llmf_gen_unit_str( "user", _filelist_str) " ,"

    _content_str    = llmf_gen_minion_content_str(minion_obj, minion_kp)
    ...
    # TODO: in some case, _maxtoken is 0, but it is not a valid value for llmf. Find out why that happened in line 72
...
function llmf_res_to_cres(llmf_resp_o, cres_o, kp,          resp_kp, delta_kp, resp_content_kp, resp_role_kp ){
    kp = ((kp != "") ? kp : SUBSEP "\"1\"")
    cres_o[ kp ] = "{"
    ...
    jdict_put( cres_o, kp SUBSEP "\"reply\"" SUBSEP "\"parts\"" SUBSEP "\"1\"", "\"text\"", llmf_resp_o[ resp_content_kp ] )
