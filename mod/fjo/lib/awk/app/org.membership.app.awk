...
function user_table_comp_init(){
    TABLE_COL_ID              = TABLE_ADD( "User ID" )
    TABLE_COL_USERNAME        = TABLE_ADD( "UserName" )
    TABLE_COL_URL             = TABLE_ADD( "Is_admin" )
}
# EndSection
...
function user_table_data_set( o, kp, text, data_offset,      obj, i, j, il, jl, _key, _dkp ){
    ...
    for (i=1; i<=il; ++i){
        ...
        for (j=1; j<=jl; ++j){
            ...
            TABLE_CELL_DEF( data_offset, TABLE_COL_URL,               user_table_juq(obj[ _dkp, "\"is_admin\"" ] ))
            ++ data_offset
        }
    }
}
