...
function user_table_comp_init(){
    ...
    TABLE_COL_URL             = TABLE_ADD( "Restricted" )
    TABLE_COL_ROLE            = TABLE_ADD( "Is_admin" )
}
# EndSection
...
function user_table_data_set( o, kp, text, data_offset,      obj, i, j, il, jl, _key, _dkp ){
    ...
    for (i=1; i<=il; ++i){
        ...
        for (j=1; j<=jl; ++j){
            ...
            TABLE_CELL_DEF( data_offset, TABLE_COL_NAME,              user_table_juq( obj[ _dkp, "\"login\"" ] ) )
            TABLE_CELL_DEF( data_offset, TABLE_COL_URL,               user_table_juq( obj[ _dkp, "\"restricted\"" ] ) )
            TABLE_CELL_DEF( data_offset, TABLE_COL_ROLE,              user_table_juq(obj[ _dkp, "\"is_admin\"" ] ) )
            ++ data_offset
        }
    }
}
...
function user_table_model_init(){
    ...
    TABLE_STATUSLINE_ADD( "c", "Add",  "Press 'c' to add a new memeber" )
