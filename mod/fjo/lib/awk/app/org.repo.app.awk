...
function user_table_comp_init(){
    ...
    TABLE_COL_VISIBILITY       = TABLE_ADD( "Private" )
    TABLE_COL_DESCRIPTION      = TABLE_ADD( "Url" )
}
# EndSection
...
function user_table_data_set( o, kp, text, data_offset,      obj, i, j, il, jl, _key, _dkp, _default_branch, _desc ){
    ...
    for (i=1; i<=il; ++i){
        ...
        for (j=1; j<=jl; ++j){
            ...
            TABLE_CELL_DEF( data_offset, TABLE_COL_VISIBILITY,            user_table_juq( obj[ _dkp, "\"private\"" ] ) )
            TABLE_CELL_DEF( data_offset, TABLE_COL_DESCRIPTION,           user_table_juq(obj[ _dkp, "\"url\"" ] ))
            ++ data_offset
        }
    }
}
