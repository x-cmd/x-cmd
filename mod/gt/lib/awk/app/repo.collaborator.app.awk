...
function user_table_data_set( o, kp, text, data_offset,      obj, i, j, il, jl, _key, _dkp ){
    ...
    for (i=1; i<=il; ++i){
        ...
        for (j=1; j<=jl; ++j){
            ...
            TABLE_CELL_DEF( data_offset, TABLE_COL_ADMIN,                   obj[ _dkp, "\"permissions\"" SUBSEP "\"admin\"" ] )
            TABLE_CELL_DEF( data_offset, TABLE_COL_PUSHPERMISSIONS,         obj[ _dkp, "\"permissions\"" SUBSEP  "\"push\"" ] )
            ++ data_offset
        }
    }
}
...
function user_table_model_init(){
    ...
    TABLE_STATUSLINE_ADD( "d", "remove",  "Press 'd' to remove a member from repo" )
