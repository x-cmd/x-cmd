...
function gemini_parse_response_data( text, obj,       _arr, _arrl, i, _current_kp, _finish_reason, _part_l, j, _kp_part_idx  ){
    ...
    for (i=1; i<=_arrl; ++i) {
        ...
        if (( _finish_reason != "" ) && ( _finish_reason != "\"STOP\"" )){
            ...
            else if ( _finish_reason == "RECITATION" )              GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": The response candidate content was flagged for recitation reasons."
            else if ( _finish_reason == "LANGUAGE" )                GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": The response candidate content was flagged for using an unsupported language."
            else if ( _finish_reason == "OTHER" )                   GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": Unknown reason."
            else if ( _finish_reason == "BLOCKLIST" )               GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": Token generation stopped because the content contains forbidden terms."
            else if ( _finish_reason == "PROHIBITED_CONTENT" )      GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": Token generation stopped for potentially containing prohibited content."
            else if ( _finish_reason == "SPII" )                    GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": Token generation stopped because the content potentially contains Sensitive Personally Identifiable Information (SPII)."
            else if ( _finish_reason == "MALFORMED_FUNCTION_CALL" ) GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": The function call generated by the model is invalid."
            else if ( _finish_reason == "IMAGE_SAFETY" )            GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": Token generation stopped because generated images contain safety violations."
            else if ( _finish_reason == "UNEXPECTED_TOOL_CALL" )    GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": Model generated a tool call but no tools were enabled in the request."
            else if ( _finish_reason == "TOO_MANY_TOOL_CALLS" )     GEMINI_RESPONSE_ERROR_MSG = _finish_reason ": Model called too many tools consecutively, thus the system exited execution."
