...
function user_table_data_set( o, kp, text, data_offset,      obj, i, j, il, jl, _key, _dkp ){
    ...
    for (i=1; i<=il; ++i){
        ...
        for (j=1; j<=jl; ++j){
            ...
            CELL_DEF( data_offset, TABLE_COL_ORGANIZATION,      juq( obj[ _dkp, "\"organization\"" SUBSEP "\"login\"" ] ) )
...
function user_statusline_normal_customize(o, kp){
    ...
    comp_statusline_data_put( o, kp, "f", "child",  "Lists the child teams of the team specified." )
}
# EndSection