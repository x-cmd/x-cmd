...
NR==2{
    ...
    gl_project_get_member_list( LOCAL_OBJ, LOCAL_COLLABORATOR_LIST )
...
NR==3{
    gl_api_init(REMOTE_OBJ)
    jiparse_after_tokenize( _, $0 );    JITER_CURLEN = 0;
    gl_api_to_member(REMOTE_OBJ, _);    delete _

    gl_project_get_member_list( REMOTE_OBJ, REMOTE_COLLABORATOR_LIST )
}
...
NR==4{
    jiparse_after_tokenize( _, $0 );    JITER_CURLEN = 0;
    gl_api_to_name( REMOTE_OBJ, _)
    gl_api_to_visibility( REMOTE_OBJ, _)
    gl_api_to_default_branch( REMOTE_OBJ, _);   delete _
