...
function gl_repo_member_get(o, arr, tmp_list,        l,a,kp ){
   l = split(tmp_list, a, ",")
    for (i=1; i<=l; ++i){
...
function gl_repo_member_add( o, username, access,          kp ){
    kp = SUBSEP Q2_1 SUBSEP Q2_MEMBER SUBSEP access
    jlist_put(o, kp, username)
}

function gl_repo_member_rm( o, username, access,          kp ){
    kp = SUBSEP Q2_1 SUBSEP Q2_MEMBER SUBSEP access
    return jlist_rm_value( o, kp, username)
}

function gl_repo_get_member_list( o, arr){
    gl_repo_member_get(o,arr,"owner,maintainer,developer,reporter,guest")
}
...
function gen_code_member_add(username, role){
    COLLAB_ADD_STR = COLLAB_ADD_STR    "\n" sh_x("gl", "repo", "member", "add", "--repo", LOCAL_REPO_NAME, "--access_level", role, username)
}
function gen_code_member_edit(username, role){
    COLLAB_EDIT_STR = COLLAB_EDIT_STR  "\n" sh_x("gl", "repo", "member", "edit", "--repo", LOCAL_REPO_NAME, "--access_level", role, username)
}
function gen_code_member_rm(username){
    COLLAB_RM_STR = COLLAB_RM_STR      "\n" sh_x("gl", "repo", "member", "rm", "--repo", LOCAL_REPO_NAME, "--yes", username)
}
