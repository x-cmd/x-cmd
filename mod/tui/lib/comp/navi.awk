...
function comp_navi_handle( o, kp, char_value, char_name, char_type,             _has_no_handle ){
    if ( o[ kp, "TYPE" ] != "navi" ) return false
        ...
        _has_no_handle = 1 - comp_navi_ctrl_col_dec(o, kp)
    else if ((ctrl_navi_sel_sw_get(o, kp) == false) && ((char_value == "l") || (char_name == U8WC_NAME_RIGHT)))
        _has_no_handle = 1 - comp_navi_ctrl_col_inc(o, kp)
    else if (comp_navi_handle_sel( o, kp, char_value, char_name, char_type)) _has_no_handle = false
...
function comp_navi_cur_col_init( o, kp ){
    o[ kp, "ctrl-col" ] = 1
    o[ kp, "trace.col.rootkp", col ] = ""
}

function comp_navi_cur_col_get( o, kp ){
    return o[ kp, "ctrl-col" ]
}

function comp_navi_cur_col_set( o, kp, val ){
    o[ kp, "ctrl-col" ] = val
}

function comp_navi_ctrl_col_inc( o, kp,     v, _rootkp ){
    v = o[ kp, "ctrl-col" ]
    ...
    comp_navi_trace_col_val_set( o, kp, ++v, _rootkp )
    o[ kp, "ctrl-col" ] = v
    return true
}

function comp_navi_ctrl_col_dec( o, kp,     v ){
    v = o[ kp, "ctrl-col" ]
    if ( v <= 1 ) return false
    o[ kp, "ctrl-col" ] = --v
    return true
...
function comp_navi_col_row_get( o, kp, col ){
    return comp_gsel_focused_item_true_get( o, kp SUBSEP "comp.sel" SUBSEP comp_navi_trace_col_val_get( o, kp, col) )
}

function comp_navi_col_row_set( o, kp, col, row,            _rootkp ){
    _rootkp = comp_navi_trace_col_val_get(o, kp, col)
    if (row > comp_navi_data_len(o, kp, _rootkp)) return false
    return ctrl_page_set(o, kp SUBSEP "comp.sel" SUBSEP _rootkp, row)
}

function comp_navi_trace_col_val_get( o, kp, col ){
    return o[ kp, "trace.col.rootkp", col ]
}

function comp_navi_trace_col_val_set( o, kp, col, val ){
    o[ kp, "trace.col.rootkp", col ] = val
}

function comp_navi_trace_row_val_get( o, kp, col, row,      rootkp ){
    rootkp = comp_navi_trace_col_val_get( o, kp, col )
    return comp_navi_data_preview_kp(o, kp, rootkp, row)
}

function comp_navi_handle_sel( o, kp, char_value, char_name, char_type,         _rootkp ){
    _rootkp = comp_navi_trace_col_val_get(o, kp, comp_navi_cur_col_get(o, kp))
    return comp_gsel_handle(o, kp SUBSEP "comp.sel" SUBSEP _rootkp, char_value, char_name, char_type)
}

function comp_navi_sel_data_add( o, kp, rootkp, val ){
...
function comp_navi_sel_sw_toggle( o, kp ){
    change_set( o, kp, "navi.body" )
    return comp_gsel_filter_sw_toggle( o, kp SUBSEP "comp.sel" SUBSEP comp_navi_trace_col_val_get( o, kp, comp_navi_cur_col_get(o, kp) ) )
}

function ctrl_navi_sel_sw_get( o, kp ) {
    return comp_gsel_filter_sw_get( o, kp SUBSEP "comp.sel" SUBSEP comp_navi_trace_col_val_get( o, kp, comp_navi_cur_col_get(o, kp) ) )
}

function comp_navi_get_cur_rootkp( o, kp,           c, r, rootkp, preview_kp ){
    if ((r = comp_navi_col_row_get( o, kp, c = comp_navi_cur_col_get(o, kp) )) == "" )   return
    rootkp = comp_navi_trace_col_val_get( o, kp, c )
    preview_kp = comp_navi_trace_row_val_get(o, kp, c, r)
    return rootkp ROOTKP_SEP preview_kp
}

function comp_navi_get_cur_preview_type( o, kp ){
    return comp_navi_get_col_preview_type( o, kp, comp_navi_cur_col_get(o, kp) )
}

function comp_navi_get_col_preview_type( o, kp, c,        r, rootkp ){
    if ((r = comp_navi_col_row_get( o, kp, c )) == "" )   return
    rootkp = comp_navi_trace_col_val_get( o, kp, c )
...
function comp_navi_data_add_kv( o, kp, rootkp, viewdata, previewdata, preview_kp, viewlen,        l ){
    ...
    comp_navi_sel_data_add( o, kp, rootkp, viewdata)
    change_set( o, kp, "navi.body")
    change_set( o, kp, "navi.footer")
}
...
function comp_navi_layout_init( o, kp, w,       i, l, _colw, _viewcol_begin ){
    w -= comp_navi_data_maxviewlen(o, kp)
    l = comp_navi_cur_col_get( o, kp )
    for (i=l; i>=1; --i){
        _colw = comp_navi_data_viewlen( o, kp, comp_navi_trace_col_val_get(o, kp, comp_navi_cur_col_get(o, kp)) )
        w -= _colw
        if (w < 0) break
        _viewcol_begin = i
    }
...
function comp_navi_paint_dim_set( o, kp, v ){  o[ kp, "IS_DIM" ] = v;    }
function comp_navi_paint_is_dim( o, kp ){  return o[ kp, "IS_DIM" ];   }
function comp_navi_change_set_all( o, kp ){
    change_set(o, kp, "navi.body")
    change_set(o, kp, "navi.footer")
    comp_navi_preview_set( o, kp )
}
...
function comp_navi_paint( o, kp, x1, x2, y1, y2, is_dim,        _comp_clear, _comp_sel, _comp_preview, _comp_box ){
    ...
    comp_navi_paint_dim_set( o, kp, is_dim )

    _comp_clear = painter_clear_screen(x1, x2, y1, y2)
    _comp_box = comp_navi_paint_box( o, kp, x1, x2, y1, y2 )
    x1++; x2--; y1+=2; y2-=2

    _comp_sel = comp_navi_paint_body( o, kp, x1, x2, y1, y2 )
    w = o[ kp, "view.body", "width" ]
    _comp_preview = comp_navi_paint_preview( o, kp, x1, x2, y1+w, y2 )

    return _comp_clear _comp_box _comp_sel _comp_preview
}

function comp_navi_paint_body( o, kp, x1, x2, y1, y2,           _rootkp, _start, _width, i, l, w, s, c ){
    _width = y2-y1+1
    comp_navi_data_maxviewlen_set( o, kp, int(_width/3))
    comp_navi_layout_init( o, kp, _width)

    _start = o[ kp, "viewcol.begin" ]
    l = comp_navi_cur_col_get( o, kp )
    for (i=_start; i<=l; ++i) {
        _rootkp = comp_navi_trace_col_val_get( o, kp, i )
        if (o[ kp, "data", _rootkp, "ava" ] != true) {
            comp_navi_unava_set( o, kp, _rootkp )
            break
        } else {
            w = comp_navi_data_viewlen( o, kp, _rootkp )
            s = s comp_navi_paint_body_sel( o, kp, _rootkp, x1, x2, y1+c, y1+c+w-2, (i != l))
            c += w
        }
    }
...
function comp_navi_paint_body_sel(o, kp, rootkp, x1, x2, y1, y2, is_dim,        s){
    s = painter_vline_ends( x1-1, x2+1, y2 ); y2--
    return s comp_navi_sel_paint( o, kp, rootkp, x1, x2, y1, y2, (comp_navi_paint_is_dim(o, kp)) ? true : is_dim )
}

function comp_navi_paint_preview( o, kp, x1, x2, y1, y2,            c, r, s, rootkp ){
    c = comp_navi_cur_col_get( o, kp )
    if ((r = comp_navi_col_row_get( o, kp, c )) == "" )   return s

    rootkp = comp_navi_get_cur_rootkp( o, kp )
    comp_navi_preview_unset( o, kp )
    if (comp_navi_cur_preview_type_is_sel( o, kp )) {
        if (o[ kp, "data", rootkp, "ava" ] != true) comp_navi_unava_set( o, kp, rootkp )
        else s = comp_navi_paint_preview_sel(o, kp, rootkp, x1, x2, y1, y2)
    } else {
        change_set( o, kp, "navi.preview" )
        comp_navi_preview_set( o, kp )
    }
    comp_navi_paint_preview_set( o, kp, rootkp, x1, x2, y1, y2)
    return s
}

function comp_navi_paint_preview_sel(o, kp, rootkp, x1, x2, y1, y2,             s){
    return comp_navi_sel_paint( o, kp, rootkp, x1, x2, y1, y2, true, true)
}

function comp_navi_sel_paint( o, kp, rootkp, x1, x2, y1, y2, is_dim, is_preview,           gkp, _comp_gsel_title, _comp_gsel_body ) {
    ...
    if (ctrl_navi_sel_sw_get(o, kp) && (rootkp == comp_navi_trace_col_val_get( o, kp, comp_navi_cur_col_get(o, kp)))) {
...
function comp_navi_paint_box( o, kp, x1, x2, y1, y2 ){
    return painter_box_arc( x1, x2, y1, y2, (comp_navi_paint_is_dim(o, kp) == true) ? UI_TEXT_DIM : TH_THEME_COLOR )
}

function comp_navi_paint_preview_set( o, kp, rootkp, x1, x2, y1, y2 ){
...
function comp_navi_preview_unset( o, kp ){
    change_unset( o, kp, "PREVIEW" )
}

function comp_navi_preview_set( o, kp ){
...
function comp_navi_current_position_set(o, kp,          c, l, _kp){
    ...
    if (c <= (l = o[ kp, "cur.pos" L ])){
        _kp = comp_navi_trace_col_val_get(o, kp, c)
        if (o[ kp, "data", _kp, "ava" ] == true){
            if (comp_navi_col_row_set(o, kp, c, o[ kp, "cur.pos", c ]) == false) return
            comp_navi_change_set_all( o, kp )
            if ((c < l) && (comp_navi_col_preview_type_is_sel(o, kp, c)) && (comp_navi_ctrl_col_inc(o, kp) == false)) return
...
function comp_navi_current_position_get(o, kp,      c, i, r, s){
    c = comp_navi_cur_col_get(o, kp)
    for (i=1; i<=c; ++i){
        r = comp_navi_col_row_get(o, kp, i)
        s = s ((s != "") ? "/" r : r)
    }
    return s
}
