...
function comp_lineeditadvise_handle( o, kp, char_value, char_name, char_type,       d ) {
    ...
    if ( char_type != U8WC_TYPE_SPECIAL ) {
        ...
        else if (char_name == U8WC_NAME_HORIZONTAL_TAB) ctrl_stredit_value_add(o, kp, comp_lineeditadvise_get_adv(o, kp))
...
function comp_lineeditadvise_paint( o, kp, x1, x2, y1, y2 ){
    return comp_lineeditadvise_paint_with_cursor_advise(o, kp, x1, x2, y1, y2)
}

function comp_lineeditadvise_paint_with_advise_box(o, kp, x1, x2, y1, y2,         rv){
    rv = comp_lineeditadvise_get_cursor_right_value(o, kp)
    comp_lineeditadvise_load_advise(o, kp, rv)
...
function comp_lineeditadvise_paint_with_cursor_advise(o, kp, x1, x2, y1, y2,       s, i, b, lv, rv, l, adv, e, _str){
    if ( ! change_is(o, kp, "lineedit") ) return
    change_unset(o, kp, "lineedit")

    s = comp_lineeditadvise_get(o, kp)
    i = comp_lineeditadvise_curpos(o, kp)
    b = ctrl_stredit_border_left_get( o, kp )
    lv = substr(s, b+1, i-b)
    rv = substr(s, i+1)
    adv = comp_lineeditadvise_get_adv(o, kp, rv)
...
function comp_lineeditadvise_has_advise(o, kp){
    return o[ kp, "advise", "has.adv" ]
}

function comp_lineeditadvise_get_adv(o, kp, rv,           adv, _completed_val){
    comp_lineeditadvise_load_advise(o, kp, rv)
...
function comp_lineeditadvise_load_advise(o, kp, rv,       s, i){
    if (rv == ""){
        s = comp_lineeditadvise_get(o, kp)
        i = comp_lineeditadvise_curpos(o, kp)
        rv = substr(s, i+1)
    ...
    if ((rv == "") || (rv ~ "^ ")){
        if ( ! comp_lineeditadvise_has_advise(o, kp) ) return
        comp_lineeditadvise_get_advise(o, kp)
    }
}

function comp_lineeditadvise_get_advise(o, kp,   s, i, fp, obj, _content, _, genv_table, lenv_table, OFFSET){
    s = comp_lineeditadvise_get(o, kp)
    i = comp_lineeditadvise_curpos(o, kp)
    kp = kp SUBSEP "advise"
    ...
    if (o[ kp, "adv.type" ] == COMP_LINEEDITADVISE_ADV_FILE) {
        comp_lineeditadvise_get_advise_fromfile(o, kp, argstr)
    } else {
        comp_lineeditadvise_get_advise_fromarr(o, kp, argstr)
    }
}

function comp_lineeditadvise_get_advise_fromarr(o, kp, argstr,            _, l, i, v, _l){
...
function comp_lineeditadvise_get_advise_fromfile(o, kp, argstr,           argarr, l, i, v, d) {
...
function comp_lineeditadvise_curpos(o, kp){ return comp_lineedit_curpos(o, kp) }
function comp_lineeditadvise_get_cursor_left_value(o, kp){ return comp_lineedit_get_cursor_left_value(o, kp) }
function comp_lineeditadvise_get_cursor_right_value(o, kp){ return comp_lineedit_get_cursor_right_value(o, kp) }
function comp_lineeditadvise_put(o, kp, val){ return comp_lineedit_put(o, kp, val) }
function comp_lineeditadvise_clear(o, kp){ return comp_lineedit_clear(o, kp) }
function comp_lineeditadvise_width_set(o, kp, w){ return comp_lineedit_width_set(o, kp, w) }
function comp_lineeditadvise_width_get(o, kp){ return comp_lineedit_width_get(o, kp) }
