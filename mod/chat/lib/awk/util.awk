...
# extra_field: none
function chat_str_truncate( str, maxl,            cutl, l, ls, rs ){
    l = length(str)
    if ( maxl <=0 ) maxl = 4096 # 2048
    cutl = int( maxl / 4 )
    if ( l <= maxl ) return str

    ls = substr(str, 1, cutl)
    if ( match( ls, "[^-a-zA-Z0-9+&@#/%?=~_|!:,.; ]+$" ) ) {
        ls = substr(ls, 1, RSTART-1)
    }

    rs = substr(str, l - cutl, cutl)
...
function chat_filelist_load_to_array(v, arr,            i, l, _, fp, fp_lower, fp_desc, fp_content, fp_base64, fp_suffix, id, _str ){
    ...
    for (i=1; i<=l; ++i){
        ...
        if (match(fp_lower, "(.png|.jpeg|.jpg|.webp|.gif)$")) {
            fp_suffix = substr(fp_lower, RSTART+1)
            ...
            if ( fp_base64 != "" ) {
                ...
                _str = _str chat_wrap_tag("error_log", "Unable to base64 encode the file.") "\n"
                arr[ fp, "type" ] = "text"
                arr[ fp, "text" ] = _str
            }
        } else if (match(fp_lower, ".pdf$")) {
            fp_suffix = substr(fp_lower, RSTART+1)
            fp_base64 = file_base64(fp)
            if ( fp_base64 != "" ) {
                arr[ fp, "type" ] = "pdf"
                arr[ fp, "text" ] = _str
                arr[ fp, "base64" ] = fp_base64
                arr[ fp, "mime_type" ] = "application/" fp_suffix
            } else {
                _str = _str chat_wrap_tag("error_log", "Unable to base64 encode the file.") "\n"
                arr[ fp, "type" ] = "text"
                arr[ fp, "text" ] = _str
