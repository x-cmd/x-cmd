...
function moonshot_gen_unit_str( role, content ){
    if ( role !~ "^\"" )    role = jqu( role )
    if ( content !~ "^\"" ) content = jqu( content )
    return "{ \"role\": " role ", \"content\": " content " }"
}

function moonshot_gen_history_str( history_obj, i,        _text_req, _text_res, _text_finishReason ){
    ...
    return  moonshot_gen_unit_str( "user", _text_req )", " moonshot_gen_unit_str( "assistant", _text_res )
}

function moonshot_gen_minion_content_str(minion_obj, minion_kp,      context, example, content){
    ...
    return moonshot_gen_unit_str( "user", context example content )
}

function moonshot_req_from_creq(history_obj, minion_obj, minion_kp,def_model,          i, l, str, \
    _system_str, _history_str, _content_str, _messages_str, _mode, _jsonmode, _maxtoken, _temperature, _data_str){
    l = chat_history_get_maxnum(history_obj, Q2_1)
    for (i=1; i<=l; ++i){
        str = moonshot_gen_history_str(history_obj, i)
        if(str != "") _history_str = _history_str str " ,"
    }

    _system_str = minion_system_tostr(minion_obj, minion_kp)
    if (_system_str != "") _system_str = moonshot_gen_unit_str( "system", _system_str ) " ,"

    _filelist_str   = minion_filelist_attach( minion_obj, minion_kp )
    if (_filelist_str != "") _filelist_str = moonshot_gen_unit_str( "user", _filelist_str) " ,"

    _content_str    = moonshot_gen_minion_content_str(minion_obj, minion_kp)
    ...
    # TODO: in some case, _maxtoken is 0, but it is not a valid value for moonshot. Find out why that happened in line 72
...
function moonshot_res_to_cres(moonshot_resp_o, cres_o, kp,          resp_kp, delta_kp, resp_content_kp, resp_role_kp ){
    kp = ((kp != "") ? kp : SUBSEP "\"1\"")
    cres_o[ kp ] = "{"
    ...
    cp_cover(cres_o, kp, moonshot_resp_o, SUBSEP "\"1\"")
    cp_cover(cres_o, kp, moonshot_resp_o, resp_kp)
    jdict_put( cres_o, kp, "\"finishReason\"", cres_o[ kp, "\"finish_reason\"" ] )
    jdict_put( cres_o, kp, "\"reply\"", "{" )
    jdict_put( cres_o, kp SUBSEP "\"reply\"", "\"role\"", moonshot_resp_o[ resp_role_kp ] )
    jdict_put( cres_o, kp SUBSEP "\"reply\"", "\"parts\"", "[" )
    jlist_put( cres_o, kp SUBSEP "\"reply\"" SUBSEP "\"parts\"", "{" )
    jdict_put( cres_o, kp SUBSEP "\"reply\"" SUBSEP "\"parts\"" SUBSEP "\"1\"", "\"text\"", moonshot_resp_o[ resp_content_kp ] )
