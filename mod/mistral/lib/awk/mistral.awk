...
function mistral_gen_unit_str( role, content ){
    if ( role !~ "^\"" )    role = jqu( role )
    if ( content !~ "^\"" ) content = jqu( content )
    return "{ \"role\": " role ", \"content\": " content " }"
}

function mistral_gen_history_str( history_obj, i,        _text_req, _text_res, _text_finishReason ){
    ...
    return  mistral_gen_unit_str( "user", _text_req )", " mistral_gen_unit_str( "assistant", _text_res )
}

function mistral_gen_minion_content_str(minion_obj, minion_kp,      context, example, content){
...
function mistral_req_from_creq(history_obj, minion_obj, minion_kp, def_model,          i, l, str, \
    ...
    for (i=1; i<=l; ++i){
        str = mistral_gen_history_str(history_obj, i)
        if(str != "") _history_str = _history_str str " ,"
    }

    _system_str = minion_system_tostr(minion_obj, minion_kp)
    if (_system_str != "") _system_str = mistral_gen_unit_str( "system", _system_str ) " ,"

    _filelist_str   = minion_filelist_attach( minion_obj, minion_kp )
    if (_filelist_str != "") _filelist_str = mistral_gen_unit_str( "user", _filelist_str) " ,"
    ...
    # TODO: in some case, _maxtoken is 0, but it is not a valid value for mistral. Find out why that happened in line 72
    _maxtoken       = (_maxtoken > 0) ? "\"max_tokens\": " _maxtoken "," : ""
    _seed           = (_seed != "") ? "\"random_seed\": " int(_seed) "," : ""
...
function mistral_res_to_cres(mistral_resp_o, cres_o, kp,          resp_kp, delta_kp, resp_content_kp, resp_role_kp ){
    kp = ((kp != "") ? kp : SUBSEP "\"1\"")
    cres_o[ kp ] = "{"
    ...
    cp_cover(cres_o, kp, mistral_resp_o, SUBSEP "\"1\"")
    cp_cover(cres_o, kp, mistral_resp_o, resp_kp)
    jdict_put( cres_o, kp, "\"finishReason\"", cres_o[ kp, "\"finish_reason\"" ] )
    jdict_put( cres_o, kp, "\"reply\"", "{" )
    jdict_put( cres_o, kp SUBSEP "\"reply\"", "\"role\"", mistral_resp_o[ resp_role_kp ] )
    jdict_put( cres_o, kp SUBSEP "\"reply\"", "\"parts\"", "[" )
    jlist_put( cres_o, kp SUBSEP "\"reply\"" SUBSEP "\"parts\"", "{" )
    jdict_put( cres_o, kp SUBSEP "\"reply\"" SUBSEP "\"parts\"" SUBSEP "\"1\"", "\"text\"", mistral_resp_o[ resp_content_kp ] )
