    ...
    for(j=1; j<=len; ++j){
        ...
        if(rule_key =="rule"){
            ...
            for(k=1; k<=rule_len; ++k){
                ...
                if( ((arg[1] ~ "^"arr[1]"$")||(arr[1] == "" )) && (arr[2] == arg[2]))   print_install_cmd_style(cmd, reference, install_name)
                else if ( (arg[1] ~ "^"arr[1]"$") && (arg[2] == "")) print_install_cmd_style(cmd, reference, install_name)
